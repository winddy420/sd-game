...
- **Curriculum content** (component/badge/phase/concept/quest text **including Markdown lesson bodies**) → `packages/content/src/locales/th.ts`, a flat `Record<string,string>` keyed by a deterministic path. **Do NOT edit by hand** — it's regenerated by merging `/tmp/sd-slices/*.th.json` (UI/quest/concept title+summary) + `/tmp/sd-bodies/*.th.json` (concept Markdown bodies). The key scheme + `localized*` resolvers live in `packages/content/src/i18n.ts` (`generateContentKeys`, `localizedQuest/Concept/Component/Badge/Phase`). In translated Markdown: prose is Thai, but code fences / inline code / commands / status codes stay verbatim.
